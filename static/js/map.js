...
(function() {
  ...
  checkCookies = function() {
    ...
    for (k in filters_types) {
      v = filters_types[k];
      if ($.cookie(k)) {
        results1.push(filters_types[k] = eval($.cookie(k)));
      } else {
  ...
  checkIfNewSpotsShouldBeLoaded = function(newPositionLat, newPositionLng, userZoomLevel) {
    var currentLat, currentLng, distance, ref;
    ref = [currentMapCenter.latitude, currentMapCenter.longitude], currentLat = ref[0], currentLng = ref[1];
  ...
  filterSpots = function() {
    ...
    $("#spots_list span.list-group-item").not("#memo_empty").each(function() {
      var ref, ref1;
      if ((ref = $(this).data("spot").is_enabled, indexOf.call(filtered_allowance, ref) < 0) || (ref1 = spot_type_lookup[$(this).data("spot").spot_type], indexOf.call([
        (function() {
          var results1;
          results1 = [];
          for (k in filters_types) {
  ...
  renderInfoWindow = function(spot, userReadOnly) {
    ...
    rating_stars = $("<br><span class='rating via_modal' data-spot-pk='" + spot.id + "'></span>").raty({
      scoreName: 'friendly_rate',
      readOnly: userReadOnly,
      score: spot.friendly_rate
    });
    return $("<div class='spot_info no_copy' data-spot-pk='" + spot.id + "'> <h4><a href='" + spot.www_url + "'>" + spot.name + "</a></h4><br> " + spot.address_street + " " + spot.address_number + "</div>").append(rating_stars)[0];
  };
  ...
  loadMarkers = function(lat, lng) {
    var jqxhr, ref, ref1, url;
    if (lat === void 0 || lng === void 0 || lat === null || lng === null) {
      ref = window.getIPbasedLocation(), lat = ref[0], lng = ref[1];
      ref1 = [Number(lat), Number(lng)], lat = ref1[0], lng = ref1[1];
    }
