...
(function() {
  var calculateDistance, checkIfEmpty, checkIfNewSpotsShouldBeLoaded, check_cookies, currentMapCenter, currentZoomLevel, desiredRadius, filterSpots, getDesiredRadius, getIPbasedLocation, hideAllInfoWindows, loadMarkers, minimumDesiredRadius, opts, spot_type_lookup, switchColumsClasses, zoomBasedIconScaleRatio,
  ...
  loadMarkers = function(lat, lng) {
    ...
    return jqxhr = $.getJSON(url, function(data) {
      ...
      $.each(data.results, function(i, spot) {
        ...
        box = $("<span class='list-group-item' id='" + spot.id + "'> <span class='badge' style='background-color:transparent'> <a href='" + spot.url + "' class='spot-details-link disabled' style='color:white'> <i class='fa fa-angle-double-right fa-2x'></i></a></span> <h4 class='list-group-item-heading'>" + spot.name + "</h4> <p class='list-group-item-text'>" + spot.address_street + " " + spot.address_number + " <span class='spot_item_details' id='" + spot.id + "'> <br><span class='glyphicon glyphicon-phone-alt'></span> " + spot.phone_number + " <a href='http://www.facebook.com/" + spot.facebook + "' target='_blank'> <i class='fa fa-facebook'></i></a> </span> </p></span>").data("spot", spot);
        rating_stars = $("<div class='rate' id='" + spot.id + "'></div>").raty({
          readOnly: false,
          score: spot.friendly_rate
        });
        contentOfInfoWindow = $("<div class='spot_info' id='" + spot.id + "'> <h4>" + spot.name + "</h4><br> " + spot.address_street + " " + spot.address_number + "</div>").append(rating_stars)[0];
        icony_allowed = {
        ...
        return window.arrMarkers[spot.id] = {
          spot: spot,
          marker: SpotMarker,
      ...
      for (k in _ref2) {
        ...
        $("#map_canvas").gmap("addMarker", spot.marker).click(function() {
          ...
          $("#map_canvas").gmap("openInfoWindow", {
            ...
            content: window.arrMarkers[this.id].info_window.content
          });
          id = this.id;
