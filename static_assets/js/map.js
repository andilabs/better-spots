...
(function() {
  var calculateDistance, checkIfEmpty, checkIfNewSpotsShouldBeLoaded, check_cookies, currentMapCenter, currentZoomLevel, desiredRadius, filterSpots, filters_allowance, filters_types, hideAllInfoWindows, loadMarkers, opts, spot_type_lookup, switchColumsCSSClasses, zoomBasedIconScaleRatio,
  ...
  switchColumsCSSClasses = function(left, right) {
  ...
  loadMarkers = function(lat, lng) {
    var id, jqxhr, k, marker, spinner, target, url, _ref;
    ...
    jqxhr = $.getJSON(url, function(data) {
      return $.each(data.results, function(i, marker) {
        ...
        contentOfInfoWindow = $("<div class='spot_info no_copy' id='" + marker.id + "'> <h4>" + marker.name + "</h4><br> " + marker.address_street + " " + marker.address_number + "</div>").append(rating_stars)[0];
        icony_allowed = {
        ...
        return SpotInfoWindow = new google.maps.InfoWindow({
  ...
  $(document).on('click', '#map_filters_button', function(e) {
    $("#map_filters input.map_filter").each(function() {
      var theCookie;
      theCookie = $.cookie($(this).attr('name'));
      if (theCookie) {
        if (theCookie === "true") {
          return $(this).prop('checked', theCookie);
        }
      } else {
